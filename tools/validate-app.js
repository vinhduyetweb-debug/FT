...
const requiredFiles = [
  ...
  "config/FTOKX_SHORT_TREND_CONFIG_180_V2_1_0.json"
];
...
async function main() {
  ...
  const config = JSON.parse(await read("config/FTOKX_SHORT_TREND_CONFIG_180_V2_1_0.json"));
  ...
  assert(app.includes('const APP_VERSION = "2.1.0"'), "App version must be 2.1.0");
  assert(sw.includes("ftokx-simple-pwa-v2.1.0-config180"), "Service worker cache version must be v2.1.0-config180");
  assert(changelog.includes("V2.1.0"), "CHANGELOG missing V2.1.0");
  assert(readme.includes("CONFIG180 WATCHLIST"), "README missing CONFIG180 WATCHLIST title");
  assert(html.includes("SHORT TREND WATCHLIST · V2.1 CONFIG180"), "Header missing V2.1 CONFIG180 badge");

  assert(app.includes("STRATEGY_PROFILE"), "Strategy profile missing");
  assert(app.includes("LÃO SHORT TREND V1.4.2"), "Config 180 strategy name missing");
  ...
  assert(app.includes("positionNotionalUsdt: 500"), "Position notional 500 missing");
  assert(app.includes("marginRequiredUsdt: 100"), "Margin 100 missing");
  ...
  console.log("VALIDATION PASSED: FTOKX SIMPLE PWA V2.1 CONFIG180 WATCHLIST package looks safe.");
}
