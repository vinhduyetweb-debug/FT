...
const STRATEGY_PROFILE = {
  id: "LAO_SHORT_TREND_V1_4_2_CONFIG_180",
  name: "LÃO SHORT TREND V1.4.2 — WATCHLIST CONFIG 180",
...
const CAPITAL_DEFAULTS = {
  futuresFundUsdt: 100,
  marginPerTradeUsdt: STRATEGY_PROFILE.marginRequiredUsdt,
  dailyMaxLossUsdt: 3,
  oneTradePerDay: true,
  paperMode: true,
  btcSweepPercent: 50,
...
const MARGIN_BY_GRADE = {
  A: STRATEGY_PROFILE.marginRequiredUsdt,
  B: STRATEGY_PROFILE.marginRequiredUsdt,
  C: STRATEGY_PROFILE.marginRequiredUsdt,
  D: STRATEGY_PROFILE.marginRequiredUsdt,
  F: STRATEGY_PROFILE.marginRequiredUsdt
};
...
function getCapitalLadder(grade, action, capitalSettings = getCapitalSettings()) {
  ...
  let label = "Config 180 dùng 100 USDT margin để tạo vị thế 500 USD, nhưng bản này chỉ WATCHLIST";
  ...
  els.ticketArea.innerHTML = `
    <section class="ticket-card decision compact-ticket ${session.action === "LOCKED_RISK" ? "locked-card" : ""}">
      <div class="compact-headline">
        <span class="signal-tier">${escapeHtml(SYMBOL_CONFIG.label)} · SHORT only · Isolated x${escapeHtml(TRADING_CONFIG.leverage)} · Limit</span>
      ...
      <details class="compact-details">
        ...
        <ol class="steps">
          <li>Chỉ dùng ${escapeHtml(SYMBOL_CONFIG.label)} Futures, SHORT only, ký quỹ cô lập, x${escapeHtml(TRADING_CONFIG.leverage)}.</li>
    ...
    <details class="panel compact-details">
      ...
      <div class="risk-grid">
        ...
        <div class="metric"><span>Vị thế danh nghĩa x5</span><strong>${escapeHtml(order.notionalUsdt)} USDT</strong></div>
...
function renderCapitalGuardPanel(session, order) {
  ...
  return `
    <details class="panel compact-details capital-guard" open>
      ...
      <p class="muted-text">Một lệnh x5 không được lớn hơn hệ thần kinh của ông. Quỹ 100 USDT thì thua một SL là nghỉ, không vào lại.</p>
    </details>
  `;
}
...
function renderFocusTicket(session, order) {
  ...
  const executeLine = isWatchlistHit
    ? "Bộ lọc khớp nhưng app đang NO_LIVE. Ghi giấy thử: Short x5, 500 USD notional, TP/SL theo Config 180, hết 12 nến 15M thì đóng quan sát."
    : canTrade
      ? "Có thể nhập tay nếu chấp nhận rủi ro. Không Market, không dời SL."
      : isLocked
        ? "App vẫn tính sẵn phiếu 100 USDT để ông nhìn lời/lỗ, nhưng trạng thái là KHÓA RỦI RO. Không nhập OKX."
        : "Có phiếu 100 USDT để nhìn bài toán, nhưng ưu tiên không xuống tiền khi Grade yếu.";
  ...
  return `
    <div class="focus-ticket ${canTrade ? "" : "focus-ticket-muted"}">
      ...
      <div class="focus-grid compact-position-grid">
        ...
        <div class="focus-metric"><span>Vị thế x5</span><strong>${escapeHtml(order.notionalUsdt)} USDT</strong></div>
...
function renderScoreDetails(session) {
  ...
  return `
    <details>
      ...
      <ul class="tech-list">
        <li>V2.1 CONFIG180 chỉ dùng BTC/USDT-SWAP, SHORT only, 4H closed candles, execution 15M, Isolated x5, vị thế 500 USD, margin 100 USDT, Grade A only, Min Fitness 92; mặc định WATCHLIST / OBSERVATION_ONLY / NO_LIVE.</li>
...
function buildCopyTicketText(session, order) {
  return [
    "FTOKX V2.1 CONFIG180 · BTC/USDT-SWAP · SHORT only · Isolated x5 · NO_LIVE",
...
function startSignalAlert(session, paperTest) {
  ...
  const title = direction === "TEST" ? "TEST CHUÔNG/RUNG" : `${direction === "SHORT" ? "BTC x5 CONFIG180 WATCHLIST SHORT" : "BTC x5 WATCHLIST " + direction}`;
